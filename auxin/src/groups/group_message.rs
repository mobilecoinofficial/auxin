...
use crate::{
	...
	message::{envelope_types::EnvelopeType, MessageSendMode},
	AuxinContext,
};

use super::{sender_key::DistributionId, GroupApiError};
...
pub enum GroupEncryptionError {
	...
	#[error(
		"Could not generate a UUID Protocol Address for our local instance! Our address is {0:?}"
	)]
	OurProtocolAddress(AuxinDeviceAddress),
	#[error(
		"Could not generate a UUID Protocol Address for selected recipient. Their address is {0:?}"
	)]
...
impl GroupSendContextV2 {
	...
	pub(crate) async fn sender_key_encrypt<R: Rng + CryptoRng>(
		&self,
		destination_address: &AuxinDeviceAddress,
		padded_plaintext: &[u8],
		context: &mut AuxinContext,
		rng: &mut R,
	) -> Result<(EnvelopeType, Vec<u8>), GroupEncryptionError> {
		let our_address = context
			.identity
			.address
			.uuid_protocol_address()
			.map_err(|_| {
				GroupEncryptionError::OurProtocolAddress(context.identity.address.clone())
			})?;
		...
		match is_sealed_sender {
			MessageSendMode::Standard => {
				...
				.await?;
				Ok((
					EnvelopeType::SenderKey,
					ciphertext_message.serialized().to_vec(),
				))
			}
			MessageSendMode::SealedSender => {
				...
				match sender_cert_maybe {
					Some(sender_certificate) => {
						...
						let destination_protocol_address =
							destination_address.uuid_protocol_address().map_err(|_| {
								GroupEncryptionError::DestProtocolAddress(
					...
					None => {
						...
						Err(GroupEncryptionError::NoSenderCertificate(
							destination_address.clone(),
						))
					}
				}
...
impl GroupSendContext {
	...
	pub async fn group_encrypt<R: Rng + CryptoRng>(
		...
		match &self {
			...
			GroupSendContext::V2(inner) => {
				inner
					.sender_key_encrypt(destination_address, padded_plaintext, context, rng)
					.await
			}
		}
	}
