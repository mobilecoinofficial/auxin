use auxin::{AuxinReceiver, address::{AuxinAddress, E164}, message::{MessageIn, MessageOut}, state::PeerInfoReply};
...
impl<'a> AppWrapper<'a> {
    ...
    pub fn retrieve_payment_address(&mut self, recipient: &AuxinAddress)
    ...
    pub fn send_text(&mut self, recipient_phone:&str, message: &str) {
        let message_struct = auxin::message::MessageOut { content: auxin::message::MessageContent::default().with_text(message.into()) };
