	...
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			GroupDecryptionError::ZkGroupError(e) => {
				write!(f, "Zero-knowledge group error: {:?}", e)
			}
			GroupDecryptionError::BincodeError(e) => write!(
				f,
				"Bincode error while attempting to decrypt group: {:?}",
				e
			),
			GroupDecryptionError::ProtobufDecodeError(e) => write!(
				f,
				"Protobuf error while attempting to decrypt group: {:?}",
				e
			),
...
impl GroupOperations {
	...
	pub fn decrypt_uuid(&self, uuid: &[u8]) -> Result<[u8; 16], GroupDecryptionError> {
		let bytes = self
			.group_secret_params
	...
	pub fn decrypt_profile_key(
		...
		Ok(self
			.group_secret_params
			.decrypt_profile_key(bincode::deserialize(profile_key)?, decrypted_uuid)?)
	}
	...
	pub fn decrypt_member(
		...
		let (uuid, profile_key) = if member.presentation.is_empty() {
			...
			let uuid = self
				.group_secret_params
				.decrypt_uuid(profile_key_credential_presentation.get_uuid_ciphertext())?;
...
impl CredentialResponse {
	pub fn parse(self) -> Result<HashMap<i64, AuthCredentialResponse>, Box<dyn std::error::Error>> {
		self.credentials
			.into_iter()
			.map(|c| {
...
pub trait CredentialsCache {
	...
	fn get(&self, key: &i64) -> Result<Option<&AuthCredentialResponse>, CredentialsCacheError>;
...
pub enum GroupApiError {
	#[error(
		"Error encountered while trying to make a request to Signal's web API for groups: {0}"
	)]
	RequestError(String),
	#[error(
		"Error encountered in response from an https request to Signal's web API for groups: {0}"
	)]
...
impl<'a, N: AuxinHttpsConnection, C: CredentialsCache> GroupsManager<'a, N, C> {
	...
	async fn get_authorization(&mut self, today: i64) -> Result<CredentialResponse, GroupApiError> {
		let today_plus_7_days = today + 7;

		let path = format!(
			"https://textsecure-service.whispersystems.org/v1/certificate/group/{}/{}",
			today, today_plus_7_days
		);

		let auth_token = self.local_identity.make_auth_header(); // This is the only place it is used.
		let req_builder =
			common_http_headers(http::Method::GET, path.as_str(), auth_token.as_str()).unwrap();

		debug!(
			"Attempting to pull in Groups authorization with request: {:?}",
			req_builder
		);
	...
	fn get_authorization_string(
		...
		let auth_credential = self
			.server_public_params
			.receive_auth_credential(*uuid.as_bytes(), today, credential_response)
			.map_err(|e| {
		...
		*/
		...
		let password = hex::encode(bincode::serialize(&auth_credential_presentation)?);
		//It appears that
	...
	pub async fn get_group(
		...
		let req_builder = common_http_headers(
			http::Method::GET,
			"https://storage.signal.org/v1/groups/",
			auth_token.as_str(),
		)
		.unwrap();
		let req = req_builder.body(Vec::default()).unwrap();
		...
		let encrypted_group = decoder.read_message().map_err(|e| {
			GroupApiError::ParsingError(format!(
				"Could not decode group response to a protobuf: {:?}",
				e
			))
		})?;
		let group_ops = GroupOperations::new(group_secret_params);
		let decrypted_group = group_ops.decrypt_group(encrypted_group)?;
...
impl std::fmt::Debug for GroupMemberInfo {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		f.debug_struct("GroupMemberInfo")
			...
			.field(
				"profile_key",
				&self.profile_key.map(|val| base64::encode(&val.get_bytes())),
			)
...
pub fn validate_group_member(
	group_member: &DecryptedMember,
...
pub fn get_group_members(group: &DecryptedGroup) -> Vec<Result<GroupMemberInfo, GroupUtilsError>> {
	...
	for group_member in group.members.iter() {
		result.push(validate_group_member(group_member));
	}
	result
}
...
pub fn get_group_members_without(
